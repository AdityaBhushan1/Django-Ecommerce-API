...
def CreateOrder(customer, oid, ammount):
    customerDetails = CustomerDetails(
        customer_id=customer.id, customer_phone=customer.phone_no
    )
    ...
    orderMeta.return_url = (
        settings.SITE_DOMAIN + "/payment_success"
    )  # Todo change this url to actual url
    ...
    try:
        api_response = Cashfree().PGCreateOrder(
            settings.CASHFREE_API_VERSION, createOrderRequest, None, None
        )
